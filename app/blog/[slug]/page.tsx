...
export async function generateMetadata({ params }: Props) {
  ...
  if (!post) return {};
  return { title: `${post.title} — ${profile.name}` };
}
    ...
    <div className="pt-[72px]">
      <Section>
        <article className="max-w-2xl">
          ...
          <h1 className="text-[28px] md:text-[40px] md:leading-[48px] font-bold tracking-tight">
            {post.title}
          </h1>
          ...
          {post.repoUrl && (
            <a
              ...
              className="inline-flex items-center gap-2 mt-4 text-sm text-text-muted hover:text-accent transition-colors duration-[var(--duration-fast)]"
            >
